...
exports._users = {
	/*
sample payload:
{"firstName":"srijita","lastName":"thakur","phone":9339072217,"password":"password123","tosAgreement":true}
	*/
	post: (data, callback) => {
		const { payload } = data;
		const firstName =
	...
	//@TODO: delete all associated checks one deleting users
				...
				if (tokenIsValid) {
					filedb.read('users', phone, (err, data) => {
						if (err) {
							...
							filedb.delete('users', phone, (err, data) => {
								callback(200);
							});
						}
					});
